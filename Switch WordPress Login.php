...
/*
 * Plugin Name: Switch WordPress Login URI
 * Plugin URI:  http://how2wordpress.usa.cc/switch-wp-login
 * Description: Switch wp-login.php to whatever you want. Switch WordPress Login URI Make Easy To Change or Rename Our WP-login.php 
 * Version:     1.0
 * Author:      How2WordPress
 * Author URI:  https://how2wordpress.usa.cc/
 * Text Domain: switch-wordpress-login
 * Domain Path:
 * Network:
 * License:     GPL-2.0+
 */

if ( defined( 'ABSPATH' ) && ! class_exists( 'switch_wordpress_login' ) ) {

	class switch_wordpress_login {
		...
		public function __construct() {
			...
			register_uninstall_hook( $this->basename(), array( 'switch_wordpress_login', 'uninstall' ) );
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Please upgrade to the latest version of WordPress to activate %s.', 'switch-wordpress-login' ), '<strong>' . __( 'Switch wordpress-login.php', 'switch-wordpress-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'Switch wordpress-login.php', 'switch-wordpress-login' ) . '</h3>';
			$out .= '<p>' . __( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'switch-wordpress-login' ) . '</p>';
			$out .= '<table class="form-table">';
				$out .= '<tr valign="top">';
					$out .= '<th scope="row">' . __( 'Networkwide default', 'switch-wordpress-login' ) . '</th>';
		...
		public function admin_init() {
			...
			add_settings_section(
				'switch-wordpress-login-section',
				__( 'Switch wordpress-login.php', 'switch-wordpress-login' ),
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'Login url', 'switch-wordpress-login' ) . '</label>',
		...
		public function rwl_section_desc() {
			...
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				$out .= '<p>' . sprintf( __( 'To set a networkwide default, go to %s.', 'switch-wordpress-login' ), '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Network Settings', 'switch-wordpress-login' ) . '</a>') . '</p>';
			}
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="updated"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'switch-wordpress-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}

		public function plugin_action_links( $links ) {
			if ( is_network_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				array_unshift( $links, '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Settings', 'switch-wordpress-login' ) . '</a>' );
			} elseif ( ! is_network_admin() ) {
				array_unshift( $links, '<a href="' . admin_url( 'options-permalink.php#rwl-page-input' ) . '">' . __( 'Settings', 'switch-wordpress-login' ) . '</a>' );
			}
		...
		public function plugins_loaded() {
			...
			load_plugin_textdomain( 'switch-wordpress-login' );
			...
			if (
				...
				wp_die( __( 'This feature is not enabled.', 'switch-wordpress-login' ) );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( 'You must log in to access the admin area.', 'switch-wordpress-login' ) );
			}
